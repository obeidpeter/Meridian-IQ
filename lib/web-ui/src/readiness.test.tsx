...
import { ReadinessList, readinessSummary, type ReadinessStep } from "./readiness";

const STEPS: ReadinessStep[] = [
  { id: "number", label: "Invoice number", state: "done", href: "#details" },
  { id: "customer", label: "Customer selected", state: "todo", href: "#details" },
  { id: "tin", label: "Customer has a TIN", state: "attention", detail: "FIRS rejects B2B paper without one." },
];
...
describe("ReadinessList", () => {
  test("sums progress and exposes it as a progressbar", () => {
    ...
    expect(screen.getByTestId("text-readiness-summary").textContent).toBe("1 of 3 ready");
  ...
  test("each step carries its state, an anchor when given, and a screen-reader suffix", () => {
    ...
    expect(screen.getByTestId("readiness-customer").getAttribute("data-state")).toBe("todo");
    ...
    expect(screen.getByText("FIRS rejects B2B paper without one.")).toBeTruthy();
  });
});
