...
export async function convertToWav(audioBuffer: Buffer): Promise<Buffer> {
  ...
  const transcodeTimeoutMs = 30_000;
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      ffmpeg.on("close", (code) => {
        clearTimeout(timer);
        if (code === 0) resolve();
        else reject(new Error(`ffmpeg exited with code ${code ?? "timeout"}`));
      });
    ...
    if (wav.length > 10 * 1024 * 1024) {
      throw new Error("Converted audio exceeds the 10MB processing limit");
    }
    return wav;
  } finally {
