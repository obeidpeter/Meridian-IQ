...
export const stampRecordsTable = pgTable(
  "stamp_records",
  {
    id: id(),
    invoiceId: uuid("invoice_id")
...
export const confirmationsTable = pgTable(
  "confirmations",
  {
    ...
    confirmingUserId: uuid("confirming_user_id").references(
      () => usersTable.id,
    ),
...
export const settlementEventsTable = pgTable(
  "settlement_events",
  {
    id: id(),
    invoiceId: uuid("invoice_id")
    ...
    // Stable provider/user-operation identity. New financial writes always set
    // this; historical rows remain null. The partial unique index is the final
    // replay wall across retries, tabs and concurrent instances.
  ...
  (t) => [
    ...
    uniqueIndex("settlement_events_external_reference_uq")
      .on(t.externalReference)
      .where(sql`${t.externalReference} IS NOT NULL`),
  ],
);
...
export const invoiceLifecycleEventsTable = pgTable(
