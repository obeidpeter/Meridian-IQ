...
export const invoicesTable = pgTable(
  ...
  (t) => [
    ...
    check("invoice_content_revision_positive", sql`${t.contentRevision} > 0`),
  ],
);
...
export const invoiceLinesTable = pgTable(
  "invoice_lines",
  {
    ...
    vatRate: numeric("vat_rate", { precision: 6, scale: 4 })
      .notNull()
      .default("0"),
    lineExtension: numeric("line_extension", {
      precision: 18,
      scale: 2,
    }).notNull(),
  ...
  (t) => [
    ...
    uniqueIndex("invoice_lines_number_uidx").on(t.invoiceId, t.lineNo),
  ],
);
