...
import { MOBILE_LOGIN_HEADERS } from "./test-client.mjs";
...
async function main() {
  ...
  const CONCURRENCY = Number(process.env.LOAD_SMOKE_CONCURRENCY ?? 4);
  ...
  if (
    !Number.isFinite(P95_GATE_MS) ||
    P95_GATE_MS <= 0 ||
    !Number.isSafeInteger(PER_ROUTE) ||
    PER_ROUTE < 1 ||
    !Number.isSafeInteger(CONCURRENCY) ||
    CONCURRENCY < 1
  ) {
    fail(
      "latency gate must be positive; request count and concurrency must be positive integers",
    );
  }
  ...
  const ROUTES = [
    ...
    {
      name: "GET /api/clerk/cases",
      path: "/api/clerk/cases",
      token: adminToken,
    },
  ];
  ...
  if (failures.length > 0) {
    fail(`${failures.length} request(s) failed; first: ${failures[0]}`);
  }
  for (const route of ROUTES) {
  ...
  if (gateBreached) {
    throw Object.assign(
      new Error(`a route's p95 exceeded ${P95_GATE_MS}ms (LOAD_SMOKE_P95_MS)`),
      { exitCode: 1 },
    );
  }
