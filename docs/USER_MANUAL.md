...
- **Your adviser's brief** (client accounts) — the firm's monthly advisory
  brief for the business: a one-line headline, a short adviser's note, and
  the position underneath in plain sections — statutory items, penalty
  exposure, VAT, money in and out, books hygiene, and (from the second
  month) what changed since last month's brief. Every number is the same
  number its own report shows ([section 5](#5-clerk--the-ai-assistant) →
  the monthly advisory brief). Appears once your firm has produced one —
  by hand from your client page, or via the monthly run once briefs are
  switched on.
...
- **Invoice Rooms** in the Work navigation is the control centre for every
  shared invoice. Search and filter by attention, awaiting response, confirmed,
  or paid; each row shows issued, opened, verified, responded, and paid progress.
- The recipient sees the canonical supplier/buyer names, invoice lines, totals,
  due date, stamp evidence, notes, payment instructions, PDF, and a shared
  activity timeline. The page never asks them to trust values copied into a
  message.
- Opening is read-only. Confirmation, query, rejection, hosted checkout,
  payment reporting, and account claiming require a six-digit code sent to the
  saved email or WhatsApp destination. Queries and rejections require a note.
- Hosted checkout appears only when its provider is configured. The ordinary
  bank-transfer instructions and immutable **Report payment** evidence remain
  available when that provider is offline or deliberately dark.
- After email verification, a new buyer may create an account and place the
  invoice in Buyer Rails. An existing account is sent to sign in instead; the
  room never resets or takes over an existing password.

The room credential is held in the link fragment, exchanged immediately for a
short-lived HttpOnly browser session, and removed from the address bar. Do not
paste it into public tickets or documents: until it expires or is revoked, the
link still grants read-only access to that invoice.
...
- **Monthly advisory brief** _(opt-in)_ — the statement's advisory sibling:
  ...
  last month's brief"** section comparing the tracked attention items
  against last month's stored brief ("1 now (was 3)"), so progress and
  slippage are visible without anyone re-reading old briefs. Clerk phrases
  only the short adviser's note at the top; every number is the section's
  own report's number, and a plain template writes the note when Clerk is
  off or the allowance is spent (the brief always says which). The firm
  sees and refreshes it on each client's console page; the client sees it
  on their dashboard ("Your adviser's brief"); once a month the sweep
  generates one for every actively engaged client and offers it over the
  client's alert channels — pointer-only, consent-gated, like every alert.
  A brief the firm generated by hand is never overwritten by the sweep.
...
- **Onboarding** — also on the drill-down: a guided, resumable checklist
  for bringing a new client onto the platform. **Start onboarding** opens
  the run; each step (identity details, consent, alert channels, first
  documents, and so on) is marked done by the platform when its evidence
  actually exists, or **skipped** by you with a recorded reason — the run
  is a record, not a to-do list on the honor system. The card also shows
  the client's **day-one opening position** — invoice history, outstanding
  receivables per currency, unpaid bills, open notices — so "where were
  they when we started" is captured before your work changes it. When the
  run completes (or you abandon it, which is also recorded), **Readiness
  report (PDF)** exports the whole story: the checklist outcomes, the
  opening position, and what stands ready — the document a firm hands its
  own client at the end of week one.
- **Advisory brief** — the client's monthly advisory brief
  ([section 5](#5-clerk--the-ai-assistant) → the monthly advisory brief),
  with **Refresh** to regenerate it from this moment's numbers and the
  month history underneath.
...
known party names and TINs are pseudonymized in the stored text first),
**retrieval quality** — when the memory feature is on, a card that replays
a fixed labelled test set against the live embedding model and scores
whether the right memory still ranks first (recall and rank metrics,
trend across runs; a nightly run can be switched on, and a drop alert
fires once per degraded run) — and
**canaries** —
...
| `INVOICE_PAYMENT_PROVIDER_URL` (+ `INVOICE_PAYMENT_PROVIDER_TOKEN`)                        | Invoice Room buyer-checkout relay; unset = hosted checkout is unavailable while bank-transfer instructions and payment reporting continue.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `INVOICE_PAYMENT_WEBHOOK_KEYS` / `INVOICE_PAYMENT_WEBHOOK_TOKEN`                           | Signed key ring, or transitional single secret, for provider confirmation at `/api/invoice-room/payments/confirm`; an empty ring makes the callback 404.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `INVOICE_ROOM_ENCRYPTION_KEY`                                                              | Dedicated 32-byte AES key (64 hexadecimal characters or base64) protecting recoverable room credentials; required in production and never exposed to a browser.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `PUBLIC_APP_URL`                                                                           | HTTPS origin used for password-reset and Invoice Room links. Required in production: the API holds readiness until it is set and safe (R112), and Invoice Room links fail closed on the same rule.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
- **e2e** — builds the API (verifying its PDF worker) and the mobile package,
  ...
  runs the accessibility matrices and bundle budgets, and drives **433
  ...
  integration, brand — the run order is load-bearing, since journeys mutate the
...
| **Invoice Room**         | A revocable, expiring, account-optional workspace for one stamped invoice. It lets the intended buyer inspect evidence and, after contact verification, respond or provide payment evidence.                                                                                                           |
