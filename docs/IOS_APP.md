...
The Valo display-name change does not change `com.meridianiq.mobile`, the
existing EAS project, or signing and push credentials. Keep those identities
intact; the [rebrand rollout checklist](valo-rebrand.md#external-rollout-checklist)
covers store listing names and push verification.
...
- Open the installed **Valo Mobile** dev build and scan the QR code shown by
...
- **Notification never appears but "sent"** — check iPhone Settings → Notifications
  → Valo Mobile is allowed; confirm the APNs key exists in `eas credentials`.
