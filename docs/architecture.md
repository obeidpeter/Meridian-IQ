...
HTTP; a transport serves only the rails it has a URL for, and failover,
recovery and the "full outage" test count served rails only.
...
open breaker — a lookup never moves it — and the rail call stays inside the
worker's bypass transaction, bounded by `RAIL_TIMEOUT_MS`.
...
credential or a garbled answer costs delay, never a failed invoice.
