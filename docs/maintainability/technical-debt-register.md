...
| P1       | Oversized server modules               | Invoice Room service exceeds 1,800 lines (pipeline.ts fell below 1,600 in R107: sweep runner, in-flight tracker and pass lock extracted) | Extract transaction-neutral policies and adapter contracts one flow at a time                         | Public API unchanged; concurrency and failure tests cover each extraction         |
