...
| P3       | Complexity ratchet                     | Met in R122: `complexity-baseline.json` records the 114 functions over 20 (maximum 117) keyed by file and function label; `pnpm run complexity:check` in the check chain and CI fails on a new hotspot or growth and advises when an entry can tighten; `complexity:write` moves the baseline down | Bring hotspots down one at a time and tighten the baseline in the same change; keep the threshold at 20 | CI complexity ratchet active and owned                                 |
