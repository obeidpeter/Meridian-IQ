...
- CI (`.github/workflows/ci.yml`): every PR runs typecheck, unit tests, a codegen-drift check, the migration rollback test against a service Postgres 16, all five frontend builds (incl. the penalty calculator), and the E2E job — `pnpm --filter @workspace/scripts run e2e` boots the built api-server + built frontends behind a path-router (`scripts/src/e2e/`) and drives 433 headless user-journey checks against a freshly seeded database. The quality gate also runs `pnpm run check` (architecture, secrets, docs, brand, the complexity ratchet against `complexity-baseline.json`), the scripts reliability/accessibility regressions, migration-only upgrade parity, a retained snapshot-consistent backup with full-catalog restore, the accessibility matrices and bundle budgets, and stamps the immutable build manifest Publish consumes (docs/operations.md). Locally it needs `DATABASE_URL` pointing at a scratch DB and the builds listed at the top of `run.mjs`.
