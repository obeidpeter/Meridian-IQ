...
- **Release gating (PL-02)**: every R2 surface checks its flag (`reconciliation`, `b2c_reporting`, `buyer_rails`, `white_label`, `erp_connectors`, `bank_feeds`, `anonymized_benchmarks`) and 404s while dark; all seven seed dark — flip via `PATCH /feature-flags/{key}` as operator.
...
- **Overnight alerts on Autoscale**: the published API runs on Autoscale, which scales to zero when idle, freezing the in-process timers. `GET /api/internal/sweep` is an idempotent wake-up trigger that runs one full timer pass synchronously (see `routes/sweep.ts`). Set a strong `SWEEP_TOKEN`, then ping it every ~5 minutes from a **Replit Scheduled Deployment** with `curl -fsS --max-time 120 --retry 3 --retry-delay 5 --retry-all-errors -H "x-op-token: $SWEEP_TOKEN" https://meridian-iq.replit.app/api/internal/sweep` (or run `scripts/sweep-ping.sh`). The endpoint is fail-closed when the token is unset and rate-limited when enabled. `/api/metrics` remains open when its ring (`METRICS_KEYS` / `METRICS_TOKEN`) is empty; when configured, callers sign the request (x-op-key-id / x-op-timestamp / x-op-signature) or pass a key in `x-op-token`. Operation credentials are never accepted in URLs; see docs/platform.md "Machine-rail credentials".
