...
spine serving SMEs, accountant firms and anchor buyers (Business Plan v3.3,
...
- Production env: `PUBLIC_APP_URL` — the origin of every link the platform
  sends; boot holds readiness without a safe https value (R112). Invoice Room
  also needs a 32-byte
  `INVOICE_ROOM_ENCRYPTION_KEY`; optional delivery and hosted-payment rails use
  `MESSAGING_WEBHOOK_*`, `INVOICE_PAYMENT_PROVIDER_*`, and
  `INVOICE_PAYMENT_WEBHOOK_KEYS` (or the legacy single token)
...
- Key R2 modules: `modules/statements` (INT-05 parser abstraction + ingestion), `modules/reconciliation` (SME-07 matcher), `modules/buyer` (BR-01/05 exposure + scoreboard), `modules/invoice-room` (account-optional secure buyer workspace; split by flow since R109 — core, supplier, buyer, payments, claim, reminders — behind an unchanged `service.ts` surface), `modules/b2c` (SME-08 clocks), `modules/connectors` (PL-03 contract + SagePro/QuickLite)
...
- `artifacts/api-server/src/bootstrap/seed.ts` — flags (release-tagged), demo tenant, buyer principals, CPD content, and opt-in development demo credentials
...
- **Release gating (PL-02)**: every R2 surface checks its flag (`reconciliation`, `b2c_reporting`, `buyer_rails`, `invoice_room`, `white_label`, `erp_connectors`, `bank_feeds`, `anonymized_benchmarks`) and 404s while dark in production; flip via `PATCH /feature-flags/{key}` as operator only after its dependencies and deployment configuration are ready.
...
- Invoice Room: a supplier creates a revocable, expiring link for a stamped invoice and monitors it from `/app/invoice-rooms`; the buyer opens `/invoice-room` without an account to inspect evidence, download the PDF, verify a saved contact, respond, pay or report payment, and optionally claim the invoice into Buyer Rails. URL credentials stay in fragments, public sessions are short-lived HttpOnly cookies, and writes are append-only and idempotent.
...
- CI (`.github/workflows/ci.yml`): every PR runs typecheck, unit tests, a codegen-drift check, the migration rollback test against a service Postgres 16, all five frontend builds (incl. the penalty calculator), and the E2E job — `pnpm --filter @workspace/scripts run e2e` boots the built api-server + built frontends behind a path-router (`scripts/src/e2e/`) and drives 433 headless user-journey checks against a freshly seeded database. The quality gate also runs `pnpm run check` (architecture, secrets, docs, brand, the complexity ratchet against `complexity-baseline.json`, the Prettier format check), the scripts reliability/accessibility regressions, migration-only upgrade parity, a retained snapshot-consistent backup with full-catalog restore, the accessibility matrices and bundle budgets, and stamps the immutable build manifest Publish consumes (docs/operations.md). Locally it needs `DATABASE_URL` pointing at a scratch DB and the builds listed at the top of `run.mjs`.
- Auth hardening: `/auth/login` rate-limits per email+IP (5 failures / 15 min → 429) plus a looser per-account cap (50 / 60 min), persisted in the Postgres `login_attempts` table on the raw pool (`modules/auth/throttle.ts`) so the caps hold cluster-wide and survive the 4xx transaction rollback; stale rows are pruned by a registered sweep. `POST /auth/change-password` (current password required, guess-throttled) with a form in the portal's signed-in panel; lost passwords are recovered via operator-issued one-time reset links (`POST /password-resets`, redeemed at the public `/auth/reset-password`). The INT-02 sweep (`modules/desk/sweeps.ts`, registered via routes/index.ts) opens one operator case per unmapped failure code, deduped by open case title.
...
- **Production DB provisioning**: Publish stages the verified CI artifact through `scripts/src/ops/replit-promote.mjs`. Under the default pilot profile (`RELEASE_PROFILE=pilot`, ADR 0004) the API build syncs the schema before the new revision serves — plain `push` (a destructive diff fails the build; it needs a reviewed migration) then the guardrail migrations. The governed profile never pushes online; `--offline-bootstrap` is its drained-traffic escape hatch (`docs/operations.md`). The guardrails in `lib/db/src/migrations/` (RLS policies, append-only triggers, `NULLS NOT DISTINCT` index) are NOT in the Drizzle schema. On every prod boot the server therefore runs `applyGuardrailMigrations` (idempotent, advisory-locked, after the port opens): all guardrail migrations are re-asserted; any migration whose target table Publish hasn't created yet is skipped and retried on a later boot (logged at warn). A read-only check then logs loudly (error level) if `meridian_tenant_isolation` / `meridian_append_only` are still missing — watch the deploy logs for both.
- **Demo identities**: demo seeding is development-only and requires both `SEED_DEMO=true` and an explicit `DEMO_PASSWORD` of at least 16 characters. Production startup disables the known demo identities and clears their local and Clerk credentials, including databases copied from development.
...
- **Invoice Room rollout:** apply schema plus guardrail migration `0048`, configure `PUBLIC_APP_URL` and `INVOICE_ROOM_ENCRYPTION_KEY`, verify the new entries under Platform ops rail configuration, then enable `invoice_room`. Production link generation fails closed when either required value is unsafe or absent; hosted checkout remains optional and bank-transfer instructions continue to work while its provider is dark.
...
- Demo accounts: staff/admin/ops/buyer plus `audit@valo.example` (read-only auditor). Credit notes only submit for clients with complete party data + layer-1 consent — in the seed that's Adaeze Foods; other demo clients lack a street and fail UBL validation with a named field.
- express JSON body limit is 8mb (5,000-row imports, statement uploads).
- **Overnight alerts on Autoscale**: the published API runs on Autoscale, which scales to zero when idle, freezing the in-process timers. `GET /api/internal/sweep` is an idempotent wake-up trigger that runs one full timer pass synchronously (see `routes/sweep.ts`) and answers 200 `ok` for a full pass, 202 `busy` (Retry-After 5) when another owner holds a pass, and 503 `partial_failure` (Retry-After 60, failure counts, no tenant data) when any sweep, the drain or the reconcile failed — the pinger retries on 202/503 and reports completion only on 200. Run `pnpm --filter @workspace/scripts run ops:sweep` every ~5 minutes from a **Replit Scheduled Deployment** with `SWEEP_URL` set to the deployment's `/api/internal/sweep` endpoint (no default host). The helper signs with `SWEEP_KEY_ID` + `SWEEP_KEY_SECRET`, the first `SWEEP_KEYS` entry, or a strong single `SWEEP_TOKEN` under key id `legacy`. The endpoint is fail-closed when its ring is unset and rate-limited when enabled. Production also fails closed when the metrics ring (`METRICS_KEYS` / `METRICS_TOKEN`) is empty. Keep `OP_LEGACY_TOKENS=off` (the production default); the single `_TOKEN` remains usable on the signed path. Operation credentials are never accepted in URLs; see docs/platform.md "Machine-rail credentials".
- See `.agents/memory/` for DB guardrails, RBAC, artifact base-path and service-worker invariants.

## Pointers

- `docs/valo-rebrand.md` — Valo naming, compatibility identifiers, and the external rollout checklist; existing deployment domain and advisory mailbox remain pending verified replacements.
- See the `pnpm-workspace` skill for workspace structure, TypeScript setup, and package details
- Requirement IDs in code comments (CORE-xx, SME-xx, BR-xx, PL-xx, INT-xx, NFR-xx, SEC-xx) refer to the original MeridianIQ Technical Requirements Document v1.1; its title and attached DOCX remain historical evidence, unchanged by the rebrand.
