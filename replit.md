...
- **Production DB provisioning**: production never runs a schema push against serving traffic — Publish stages the verified CI artifact through `scripts/src/ops/replit-promote.mjs` and schema changes arrive only as reviewed versioned migrations under the release plan (`docs/operations.md`); `--offline-bootstrap` is the drained-traffic escape hatch. The guardrails in `lib/db/src/migrations/` (RLS policies, append-only triggers, `NULLS NOT DISTINCT` index) are NOT in the Drizzle schema. On every prod boot the server therefore runs `applyGuardrailMigrations` (idempotent, advisory-locked, after the port opens): all guardrail migrations are re-asserted; any migration whose target table Publish hasn't created yet is skipped and retried on a later boot (logged at warn). A read-only check then logs loudly (error level) if `meridian_tenant_isolation` / `meridian_append_only` are still missing — watch the deploy logs for both.
