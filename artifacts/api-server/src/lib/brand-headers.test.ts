...
for (const suffix of ["csrf", "workspace", "client"] as const) {
  test(`${suffix}: accepts Valo, historical, and matching dual headers`, () => {
    const value =
      suffix === "csrf" ? "1" : suffix === "client" ? "mobile" : "buyer";
    assert.equal(
      brandHeader(request({ [`x-valo-${suffix}`]: value }), suffix),
      value,
    );
    assert.equal(
      brandHeader(request({ [`x-meridian-${suffix}`]: value }), suffix),
      value,
    );
    assert.equal(
      brandHeader(
    ...
    assert.throws(
      () =>
        brandHeader(
          request({
            [`x-valo-${suffix}`]: "buyer",
            [`x-meridian-${suffix}`]: "other",
          }),
          suffix,
        ),
      /Conflicting client headers/,
    );
  });
}
