...
// The phrasing eval corpus (split from phrasing-eval.ts, the
// eval-fixtures.ts precedent): the pack registry and the FIXED synthetic
// fact packs the eval lane replays. Every fixture is invented data — no
// client content may ever enter this file. The runner, deterministic
// scoring and the canary stay in phrasing-eval.ts.
...
// promptVersions map all derive from this list. SURFACE_PURPOSE
// (phrasing-eval.ts) stays hand-written (purpose names are a closed
// gateway catalogue, not mechanical derivations) but is Record-typed so an
// eighth pack fails to compile until its purpose entry exists.
export const PHRASING_PACKS = [
  DIGEST_PHRASING,
  CHASER_PHRASING,
  STATEMENT_PHRASING,
  VAT_NOTE_PHRASING,
...
export const PHRASING_FIXTURES: PhrasingFixture[] = [
  ...
  // the injection fixture plants its payload there.
  {
    ...
    facts: {
      cause:
        "The buyer's Tax Identification Number failed validation against the national register.",
  ...
  },
  {
    ...
    facts: {
      ...
      example: null,
      escalationReason:
        "Why is my invoice stuck? Nothing seems to be happening.",
    },
  },
  ...
  // snapshots of computeAdvisoryBriefSections' shape.
  {
    ...
    facts: [
      briefSection(
        "statutory",
        "Statutory position",
        "Filings register & compliance calendar",
        [
          briefFact(
          ...
          briefFact(
            ...
            "VAT 2026-05",
            "text",
            "due 2026-06-21 (upcoming)",
          ),
          briefFact(
            "open_filing_2",
            "VAT 2026-06",
            "text",
            "due 2026-07-21 (prepared)",
          ),
        ],
      ),
  ...
  },
  {
    ...
    facts: [
      ...
      briefSection(
        "changes",
        "Since last month's brief",
        "Previous month's advisory brief",
        [
          briefFact(
            "improved_count",
