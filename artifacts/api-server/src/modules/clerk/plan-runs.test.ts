...
test("a chaser section refuses whole-plan approval; duplicates 409; the wire shape parses", async () => {
  // draft_chasers is not plan-runnable (round-32 review M3): the drafts
  // would be generated by a worker and handed to nobody.
  const chaserCase = await seedCase(
    caseAnswerWith([
      {
        kind: "submit_overdue",
        clientPartyId: clientA,
      ...
      { kind: "draft_chasers", clientPartyId: clientA, invoiceIds: [overdue1] },
    ]),
  );
  await assert.rejects(
    createPlanRunFromCase(chaserCase, principal),
    /approved individually/i,
  );

  // One LIVE run per case: the second whole-plan approval answers 409.
  const dupCase = await seedCase(
    caseAnswerWith([
      { kind: "retry_failed", clientPartyId: clientA, invoiceIds: [overdue1] },
    ]),
  );
  const first = await createPlanRunFromCase(dupCase, principal);
  await assert
    .rejects(
      createPlanRunFromCase(dupCase, principal),
      /already queued or running/i,
    )
    .catch(async (err) => {
      // The first run's kick may have already finished it (terminal frees
      // the slot) — then the second create legitimately succeeds. Accept
      // either; the invariant is "never two LIVE runs", which the unique
      // index enforces at the database.
      const settled = await loadRun(first.id);
      assert.ok(["done", "halted"].includes(settled.status), String(err));
    });
  await driveToTerminal(first.id);

  // The wire shape parses against the generated response zod — the exact
  // drift that shipped (and 500'd) in review M1.
  const { GetPlanRunResponse } = await import("@workspace/api-zod");
  const settled = await loadRun(first.id);
  const stripped = {
    id: settled.id,
    firmId: settled.firmId,
    caseId: settled.caseId,
    templateKey: settled.templateKey,
    ...
    processedSteps: settled.processedSteps,
    haltReason: settled.haltReason,
    approvedBy: settled.approvedBy,
    createdAt: settled.createdAt,
    updatedAt: settled.updatedAt,
  };
  const parsed = GetPlanRunResponse.safeParse(stripped);
...
test("losing the approver's capability halts the run; remaining steps skip", async () => {
  ...
  try {
    ...
    assert.equal(
      settled.steps[1].decisionId,
      null,
      "skipped steps ran nothing",
    );
  } finally {
