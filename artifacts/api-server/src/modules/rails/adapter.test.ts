...
test("the breaker opens after three transient errors, keeps its outage start across a failed probe, and re-arms retry-at", async () => {
  ...
  try {
    ...
    assert.equal(
      (await breakerStatus("rail_primary")).state,
      "closed",
    ...
    assert.equal(
      opened.circuitOpen,
      false,
      "the third call still reached the rails before they tripped",
    );
    ...
    assert.ok(
      primary.retryAt && primary.retryAt.getTime() > Date.now(),
      "a retry-at is armed",
    );
    assert.ok(
      primary.retryAt!.getTime() <= Date.now() + railOpenCooldownMs() + 1_000,
    );
    ...
    assert.deepEqual(
      refused.tried.map((t) => t.raw.circuit),
      ["open", "open"],
    );
    ...
    assert.equal(
      after.openedAt?.getTime(),
      primary.openedAt?.getTime(),
      "same outage instance",
    );
    assert.ok(
      after.retryAt!.getTime() > probeAt.getTime(),
      "retry-at re-armed",
    );
    ...
    assert.deepEqual(
      [closed.state, closed.failureCount, closed.openedAt, closed.retryAt],
      ["closed", 0, null, null],
    );
  } finally {
    setRailTransport(null);
...
test("resolution: the environment lights the HTTP transport, a bound transport wins, null restores env resolution", async () => {
  try {
    ...
    assert.deepEqual(railTransportSummary(), {
      ...
      environment: "sandbox",
      rails: {
        rail_primary: { configured: true },
        rail_secondary: { configured: true },
      },
    });
    ...
    assert.equal(
      currentRailTransport(),
      http,
      "memoised while the env tuple is unchanged",
    );
    assert.deepEqual(railTransportSummary(), {
      transport: "http",
      environment: "live",
      rails: {
        rail_primary: { configured: true },
        rail_secondary: { configured: false },
      },
    });
    ...
    assert.notEqual(
      rebuilt,
      http,
      "a changed env tuple rebuilds the transport",
    );
    ...
    assert.equal(
      previous,
      rebuilt,
      "setRailTransport hands back what resolution would have used",
    );
    ...
    setRailTransport(null);
    assert.equal(
      currentRailTransport().name,
      "http",
      "null restores ENV resolution, not the simulator",
    );
...
test("a transport that serves one rail: failover never counts the other rail, and one open breaker is a full outage", async () => {
  ...
  try {
    ...
    assert.equal(
      first.tried.length,
      1,
      "rail_secondary is not served, so it was never tried",
    );
    ...
    assert.equal(
      parked.circuitOpen,
      true,
      "the only served rail refused, so every breaker is open",
    );
...
test("a lookup honours an open breaker but never moves it: the next SUBMIT is the probe", async () => {
  ...
  try {
    ...
    assert.equal(
      (await breakerStatus("rail_primary")).state,
      "open",
      "a null lookup leaves the breaker where it was",
    );
    ...
    assert.equal(
      probe.tried.length,
      1,
      "rail_primary took the probe; rail_secondary stayed refused",
    );
    assert.equal(
      (await breakerStatus("rail_primary")).state,
      "closed",
      "the submit was the probe and it closed the breaker",
    );
  } finally {
    setRailTransport(null);
    await closeBreakers();
...
test("a lookup error counts against that rail's breaker and is re-thrown only when no rail held the stamp; a hit anywhere wins; three errors open the breaker", async () => {
  ...
  try {
    ...
    fake.script(INV.invoiceNumber, {
      op: "lookup",
      outcome: "unavailable",
      times: 1,
    });
    await assert.rejects(recoverExistingStamp(INV, key), (err: unknown) => {
      assert.ok(
        err instanceof RailLookupError,
        "a RailLookupError, not a plain throw",
      );
    ...
    assert.deepEqual(
      [
        (await breakerStatus("rail_secondary")).state,
        (await breakerStatus("rail_secondary")).failureCount,
      ],
      ["closed", 0],
    );
    ...
    assert.equal(
      (await recoverExistingStamp(INV, key))?.rail,
      "rail_secondary",
    );
    const primary = await breakerStatus("rail_primary");
...
test("submitWithFailover reports the rails SENT apart from the rails TRIED: a breaker refusal is tried, never sent", async () => {
  ...
  try {
    ...
    const { result, tried, sent, circuitOpen } = await submitWithFailover(
      INV,
      key,
    );
    ...
    assert.deepEqual(
      tried[0]?.raw,
      { circuit: "open" },
      "the refusal is on the tried list",
    );
    assert.equal(tried[0]?.rail, "rail_primary");
    assert.equal(
      sent.length,
      1,
      "only the rail actually called is on the sent list",
    );
...
test("the scripted fake runs a scripted rejection code through the shared sanitiser: garbage falls back to MBS_SCHEMA_INVALID, a wire-shaped code passes", async () => {
  ...
  try {
    ...
    assert.equal(
      garbage.result.errorCode,
      "MBS_SCHEMA_INVALID",
      "spaces and punctuation the wire never carries",
    );
    assert.equal(
      garbage.result.raw.code,
      "MBS_SCHEMA_INVALID",
      "raw carries the sanitised code, not the script's",
    );
    ...
    assert.equal(
      wire.result.errorCode,
      "E-1001",
      "an access point's own reference passes through",
    );
  } finally {
    setRailTransport(null);
    await closeBreakers();
...
test("probe slot: two workers on a due-open breaker — exactly one probes, the other is refused until the lease ends", async () => {
  ...
  try {
    ...
    assert.equal(
      during.state,
      "half_open",
      "the first worker holds the probe slot",
    );
    ...
    assert.equal(
      second.circuitOpen,
      true,
      "the second worker is refused while the probe is in flight",
    );
    assert.ok(
      second.retryAfter && second.retryAfter.getTime() > Date.now(),
      "and told when the lease ends",
    );
...
test("probe slot: a probe that died with the slot is taken over once its lease passes; a failed probe records its code and re-arms", async () => {
  ...
  try {
    ...
    assert.equal(
      after.probeStartedAt,
      null,
      "the slot is released with the verdict",
    );
    assert.ok(
      after.retryAt && after.retryAt.getTime() > Date.now(),
      "retry-at re-armed",
    );
    assert.ok(
