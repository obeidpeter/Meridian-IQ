...
test("PUT /__fake/script validates and queues; a fault expires after `times` calls", async () => {
  const unknown = await control("PUT", "/__fake/script", {
    invoiceNumber: "X",
    outcome: "explode",
  });
...
test("an accepted key is held: re-sent it is 409, looked up it is the same stamp; DELETE /__fake forgets it", async () => {
  ...
  const found = await fetch(
    `${fake.url}/v0/submissions/${encodeURIComponent(key)}`,
    {
      headers: { authorization: `Bearer ${TOKEN}` },
    },
  );
  ...
  const health = (await (await control("GET", "/__fake/healthz")).json()) as {
    held: number;
    calls: number;
  };
...
test("a scripted duplicate is recoverable only with holdsStamp", async () => {
  ...
  await control("PUT", "/__fake/script", {
    invoiceNumber: orphan.inv.invoiceNumber,
    outcome: "duplicate",
  });
  const d1 = await post({
  ...
  const miss = await fetch(
    `${fake.url}/v0/submissions/${encodeURIComponent(orphan.key)}`,
    {
      headers: { authorization: `Bearer ${TOKEN}` },
    },
  );
  ...
  const hit = await fetch(
    `${fake.url}/v0/submissions/${encodeURIComponent(held.key)}`,
    {
      headers: { authorization: `Bearer ${TOKEN}` },
    },
  );
...
test("credentials are enforced on submit and lookup; the calls log says so", async () => {
  ...
  const noAuth = await post(
    { idempotencyKey: key, rail: "rail_primary", invoice: inv },
    {},
  );
  ...
  await wrong.text();
  const log = (await (await control("GET", "/__fake/calls")).json()) as Array<{
    authorized: boolean;
    outcome: string;
  }>;
...
test("a lookup-op script faults the GET, never the POST: 503, 401, 429 and a 200 that is not a stamp", async () => {
  const cells: Array<
    [
      "unavailable" | "unauthorized" | "rate_limit" | "malformed",
      number,
      string | null,
    ]
  > = [
  ...
  for (const [outcome, status, code] of cells) {
    ...
    assert.equal(
      posted.status,
      201,
      `${outcome}: the submit is untouched by a lookup-op script`,
    );
    await posted.text();
    ...
    assert.equal(
      again.status,
      200,
      `${outcome}: once consumed the stamp the rail holds is answered`,
    );
...
test("malformed with holdsStamp: the submit answers 200 garbage, a re-send is a real 409 and the GET answers the stamp", async () => {
  ...
  assert.equal(
    again.status,
    409,
    "the rail DID stamp: the re-send is a duplicate",
  );
  assert.equal(
    ((await again.json()) as { code: string }).code,
    "MBS_DUPLICATE",
...
test("a scripted rate_limit carries the Retry-After it was given, in-process and over the control endpoint", async () => {
  ...
  assert.equal(
    ((await ra.json()) as { code: string }).code,
    "RAIL_RATE_LIMITED",
  );
...
test("a 2 MB submission is 413 with connection: close, and the next request on a fresh fetch succeeds", async () => {
  ...
  assert.equal(
    ((await huge.json()) as { code: string }).code,
    "PAYLOAD_TOO_LARGE",
  );
  ...
  assert.equal(
    next.status,
    201,
    "the rail still serves after dropping that connection",
  );
  await next.text();
...
test(
  "20 oversized rejections preserve the next GET and allow a same-port restart",
  { timeout: 15_000 },
  async () => {
    ...
    try {
      for (let i = 0; i < 20; i++) {
        ...
        const next = await fetch(`${rail.url}/__fake/healthz`, {
          signal: AbortSignal.timeout(3_000),
        });
...
test(
  "a rejected unfinished upload closes within a fixed deadline and releases its port",
  { timeout: 5_000 },
  async () => {
    ...
    try {
      ...
      await Promise.race([
        ...
        new Promise<never>((_resolve, reject) => {
          deadline = setTimeout(
            () => reject(new Error("Rejected upload stayed open")),
            2_500,
          );
        }),
      ]);
...
test("PUT /__fake/script accepts op lookup and refuses an unknown op", async () => {
  const bad = await control("PUT", "/__fake/script", {
    invoiceNumber: "X",
    outcome: "unavailable",
    op: "nope",
  });
  ...
  assert.equal(
    posted.status,
    201,
    "a lookup-op script leaves the submit alone",
  );
  await posted.text();
  const got = await lookup(key);
