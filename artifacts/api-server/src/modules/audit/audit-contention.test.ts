...
import { pool, db, getDb, runInBypassContext, auditEventsTable } from "@workspace/db";
...
test("audit lock deadlines bound contention without appending or forking evidence", async () => {
  ...
  const event = { action: "test.audit-contention", entityType: "test", entityId };
  try {
    await blocker.query("BEGIN");
    await blocker.query("SELECT pg_advisory_xact_lock(918273)");
    const started = Date.now();
    await assert.rejects(runInBypassContext(async () => {
      await getDb().execute(sql`SET LOCAL lock_timeout = '100ms'`);
      await appendAudit(event);
    }));
    assert.ok(Date.now() - started < 2_000);
    assert.equal((await db.select().from(auditEventsTable).where(eq(auditEventsTable.entityId, entityId))).length, 0);
  } finally { await blocker.query("ROLLBACK"); blocker.release(); }
  const row = await appendAudit(event);
  const next = await appendAudit({ ...event, entityId: randomUUID() });
  assert.equal(next.prevHash, row.hash, "the global evidence chain is preserved");
