...
async function main(): Promise<void> {
  ...
  await runInBypassContext(async () => {
    await getDb()
      .insert(firmsTable)
      .values({ id: firmId, name: `bench-${firmId.slice(0, 8)}` });
    await getDb()
      .insert(partiesTable)
      .values([
        {
          ...
          legalName: "Bench Supplier Ltd",
          tin: `bench-${firmId.slice(0, 8)}-s`,
          tinValidated: true,
          street: "1 Bench Road",
          city: "Lagos",
        },
        {
          id: buyerId,
          type: "buyer",
          legalName: "Bench Buyer Ltd",
          tin: `bench-${firmId.slice(0, 8)}-b`,
          tinValidated: true,
          street: "2 Bench Road",
          city: "Lagos",
        },
  ...
  console.log(
    `Draining ${N} submissions through the worker + simulated rails...`,
  );
  ...
  const stamped = await runInBypassContext(async () => {
    ...
    const list =
      (rows as unknown as { rows?: { count: string }[] }).rows ??
  ...
  console.log("");
  console.log(
    `processed:      ${processed}/${N} outbox events in ${(elapsedMs / 1000).toFixed(1)}s`,
  );
  console.log(`stamped:        ${stamped}`);
  console.log(
    `throughput:     ${perSecond.toFixed(1)} submissions/s (single worker)`,
  );
  console.log(`extrapolated:   ${perDay.toLocaleString()} submissions/day`);
  console.log(
    `NFR-03 target:  ${TARGET_PER_DAY.toLocaleString()}/day -> headroom x${(perDay / TARGET_PER_DAY).toFixed(1)}`,
  );
