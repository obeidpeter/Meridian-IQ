...
import { createLoopbackClient } from "./loopback-client.ts";
...
test("fixture sources are distinct real TCP addresses, stable across requests, with intact cookies and headers", async (t) => {
  const base = await serve(t, (req, res) => {
    res.setHeader("set-cookie", ["session=one; HttpOnly", "other=two; HttpOnly"]);
    res.end(JSON.stringify({ ip: req.socket.remoteAddress, forwarded: req.headers["x-forwarded-for"] ?? null, csrf: req.headers["x-valo-csrf"] }));
  });
  ...
  for (const client of [first, second, first]) {
    const response = await client.request(base, { headers: { "x-valo-csrf": "1" } });
    assert.equal(response.status, 200);
    assert.deepEqual(await response.json(), { ip: client.address, forwarded: null, csrf: "1" });
    assert.deepEqual(response.headers.getSetCookie(), ["session=one; HttpOnly", "other=two; HttpOnly"]);
  }
});
...
test("fixture requests preserve JSON bytes and bodyless responses", async (t) => {
  ...
  const response = await client.request(base, { method: "POST", body, headers: { "content-type": "application/json" } });
...
test("fixture client rejects non-loopback URLs without connecting", async () => {
  ...
  const client = createLoopbackClient();
  try {
    for (const url of ["https://127.0.0.1", "http://example.test", "http://user:password@127.0.0.1"]) {
...
test("fixture deadlines and close release stalled requests", { timeout: 5_000 }, async (t) => {
  let arrived!: () => void;
  const arrival = new Promise<void>((resolve) => { arrived = resolve; });
  let requests = 0;
  const base = await serve(t, () => { if (++requests === 2) arrived(); });
  const timed = createLoopbackClient(100);
  const closed = createLoopbackClient();
  t.after(() => { timed.close(); closed.close(); });
