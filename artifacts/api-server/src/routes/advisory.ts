...
router.post("/assessments", async (req, res): Promise<void> => {
  ...
  const parsed = RunAssessmentBody.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.message });
    return;
  }
  const result = computeAssessment(parsed.data.answers);
  const completedAt = new Date();
  const title =
    parsed.data.title ?? `Readiness assessment (${result.band}, ${result.score}%)`;
  const findings = {
...
router.post("/vat-risk/analyze", async (req, res): Promise<void> => {
  ...
  const parsed = AnalyzeVatRiskBody.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.message });
    return;
  }
  const rows = parsed.data.csv
    ? parseLedgerCsv(parsed.data.csv)
    : (parsed.data.rows ?? []);
  if (rows.length === 0) {
    res
      .status(400)
  ...
  const report = analyzeLedger(rows, validStamps, parsed.data.buyerName);

  let engagementId: string | null = null;
  if (parsed.data.clientPartyId) {
    const [row] = await getDb()
      .insert(engagementsTable)
      .values({
        firmId,
