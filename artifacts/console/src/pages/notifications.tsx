...
import { Bell, CheckCheck, MailCheck, Radio } from "lucide-react";
import {
  Metric,
  MetricStrip,
  SegmentedControl,
...
function entityHref(item: NotificationFeedItemsItem) {
  ...
  if (item.entityType === "clerk_case") return "/clerk";
  if (item.entityType === "billing_statement") return "/statements";
  return null;
}
...
export function Notifications() {
  ...
  const exceptions = items.length - delivered;
  ...
  return (
    <div className="space-y-6">
      <WorkspaceHeader
        eyebrow="Inbox"
        title="Notifications"
        description="Firm delivery history, read state and links back to operational records."
        actions={
