...
describe("memberFlags", () => {
  ...
  test("firm roles without MFA and accounts never used are flagged", () => {
    ...
    expect(memberFlags({ ...base, lastSignInAt: null })).toEqual([
      "Never signed in",
    ]);
    expect(
      memberFlags({ ...base, mfaEnabled: false, lastSignInAt: null }),
    ).toEqual(["No MFA", "Never signed in"]);
  });
  test("client users are not flagged for MFA", () => {
    expect(
      memberFlags({ ...base, role: "client_user", mfaEnabled: false }),
    ).toEqual([]);
  });
});
