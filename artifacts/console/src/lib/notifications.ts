// Pure display helpers for the notification bell's feed. This module is a
// deliberate MIRROR of the SME app's src/lib/notifications.ts — the two apps
// have no shared UI lib, so the vocabulary (channel labels, chip tones, the
// relative-time buckets) is duplicated here and pinned by a parity test in
// each app (the mobile chip-parity idiom). Change one, change both.
