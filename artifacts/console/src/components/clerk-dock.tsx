...
function DockAnswer({ answer }: { answer: ClerkAnswer }) {
  if (!answer.answered) {
    return (
  ...
  return (
    <div className="space-y-3 rounded-md border border-slate-200 bg-slate-50 p-3">
      ...
      <p className="flex items-center gap-1.5 text-[11px] font-semibold text-teal-800">
        ...
        Grounded in approved claims and live firm records
      </p>
    </div>
  );
}
...
export function ClerkDock() {
  ...
  const context = workspaceContext(location);
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        ...
        >
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-5">
            ...
            {answer && <DockAnswer answer={answer} />}
            {ask.isError && (
